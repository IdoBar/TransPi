...
if (params.onlyAsm || params.onlyAnn || params.onlyEvi || params.all) {
    ...
    if (params.onlyAsm || params.all) {
        ...
        if (!params.skipQC) {
            ...
            process fasqc {
                ...
                script:
                    ...
                    echo "FastQC: \$( fastqc --version | sed -e "s/FastQC v//g" )" >fastqc.version.txt
                    """
            }
        }
        ...
        if (!params.skipFilter) {
            ...
            process fastp {
                ...
                script:
                    ...
                    echo "fastp: \$( fastp --version 2>&1 | awk '{print \$2}' )" >fastp.version.txt
                    """
            }
        ...
        if (params.rRNAfilter) {
            ...
            process remove_rrna {
                ...
                if (!params.skipFilter) {
                    ...
                    echo "SortMeRNA: \$( sortmerna -version | grep "SortMeRNA version" | awk '{print \$3}' )" >sortmerna.version.txt
                    """
                } else {
                    """
                    sortmerna --ref ${params.rRNAdb} --reads ${reads[0]} --reads ${reads[1]} --threads ${task.cpus} --aligned rRNAreads --other nonrRNAreads --paired_in --out2 --fastx --workdir .
        ...
        process trinity_assembly {
            ...
            script:
                ...
                echo "Trinity: \$( Trinity --version | grep "version" | head -n 1 | cut -f 2 -d "-" | tr -d "v" )" > trinity.version.txt
        ...
        process soap_assembly {
            ...
            script:
                ...
                echo "SOAP: \$( SOAPdenovo-Trans-127mer --version | grep "version" | awk '{print \$2,\$3}' | cut -f 1 -d ":" | cut -f 2 -d " " )" >soap.version.txt
		        exit 0
                """
        }
        ...
        process velvet_oases_assembly {
            ...
            script:
                ...
                echo -e "\\n-- Replacing -Velvet- from contig names --\\n"
                for x in `echo $k | tr "," " "`;do
                    sed -i "s/>/>Velvet.k\${x}./g" oases.\${x}/contigs.fa
                done
                echo -e "\\n-- Concatenating contigs --\\n"
                cat oases.*/contigs.fa >${sample_id}.Velvet.fa
                
                echo -e "\\n-- Copy concatenatied assemblies for each kmer --\\n"
                for x in `echo $k | tr "," " "`;do
                ...
                rm -rf oases.*
                echo -e "\\n-- Write versions --\\n"
                printf "Velveth: \$( velveth | grep "Version" | cut -f 2 -d " " )\\nVelvetg: \$( velvetg | grep "Version" | cut -f 2 -d " " )\\nOases: \$( oases | grep "Version" | cut -f 2 -d " " )\\n" > velvet_oases.versions.txt

                # v=\$( velveth | grep "Version" | cut -f 2 -d " " )
                # echo "Velveth: \$( velveth | grep "Version" | cut -f 2 -d " " )" > velveth_oases.versions.txt
                # v1=\$( velvetg | grep "Version" | cut -f 2 -d " " )
                # echo "Velvetg: \$v1" > velvetg_oases.versions.txt
                # v2=\$( oases | grep "Version" | cut -f 2 -d " " )
                # echo "Oases: \$v2" > oases.versions.txt
                # cat velveth_oases.versions.txt velvetg_oases.versions.txt oases.versions.txt > velvet_oases.versions.txt
                echo -e "\\n-- Finished process --\\n"
		        exit 0
                """
        }
        ...
        process rna_spades_assembly {
            ...
            script:
                ...
                echo "rna-SPADES: \$( rnaspades.py -v 2>&1 | awk '{print \$4}' | tr -d "v" )" >rna_spades.version.txt
		        exit 0
                """
        }
        ...
        process transabyss_assembly {
            ...
            script:
                ...
                echo "Trans-ABySS: \$( transabyss --version )" >transabyss.version.txt
		        exit 0
                """
    ...
    if (params.onlyEvi || params.onlyAsm || params.all) {
        ...
        process evigene {
            ...
            script:
                def mem_MB=(task.memory.toMega())
                if ((workflow.containerEngine == 'singularity' || workflow.containerEngine == 'docker' || workflow.containerEngine == 'apptainer' ) && params.oneContainer) {
                    """
                    echo -e "\\n-- Starting EviGene --\\n"
        ...
        process rna_quast {
            ...
            script:
                ...
                echo -e "\\n-- Preparing rnaQuast results file --\\n"
        ...
        process busco4 {
            ...
            script:
                ...
                # v=\$( busco -v | cut -f 2 -d " " )
                printf "BUSCO4: \$( busco -v | cut -f 2 -d " " )\\n" > busco4.version.txt
                """
        }
    ...
    if (params.onlyAsm || params.all) {
        ...
        process mapping_trinity {
            ...
            script:
                ...
                printf "Bowtie2: \$( bowtie2 --version | head -n1 | cut -f 3 -d " " )\\n" > bowtie2.version.txt
                """
        }
    ...
    if (params.onlyAnn || params.all) {
        ...
        if (params.shortTransdecoder) {
            ...
            process transdecoder_short {
                ...
                script:
                    ...
                    echo "Transdecoder: \$( TransDecoder.LongOrfs --version | cut -f 2 -d " " )" >transdecoder.version.txt
                    """
                }
        ...
        process swiss_diamond_trinotate {
            ...
            script:
                ...
                echo "Diamond: \$( diamond --version 2>&1 | tail -n 1 | cut -f 3 -d " " )" >diamond.version.txt
                """
        }
        ...
        process hmmer_trinotate {
            ...
            script:
                ...
                echo "HMMER: \$( hmmsearch -h | head -n 2 | cut -f 3 -d " " | grep [0-9] )" > hmmer.version.txt
                """
        }
        ...
        if (params.withSignalP) {
            ...
            process signalP_trinotate {
                ...
                script:
                    ...
                    ${params.signalp} -T \$TMPDIR -f short -n ${sample_id}.signalp.out ${transdecoder_pep}
        ...
        if (params.withRnammer) {
            ...
            process rnammer_trinotate {
                ...
                shell:
                    ...
                    sed 's|FindBin::RealBin/util|FindBin::RealBin|g' /opt/conda/envs/TransPi/bin/RnammerTranscriptome.pl > ./RnammerTranscriptome.pl
                    chmod +x ./RnammerTranscriptome.pl
                    # source activate /opt/conda/envs/TransPi
        ...
        process trinotate {
            ...
            publishDir "${params.outdir}/trinotate", mode: "copy", overwrite: true, pattern: "*.{terms.txt,xls,sqlite,gene_trans_map}"
            publishDir "${workDir}/.versions", mode: "copy", overwrite: true, pattern: "*.version.txt"
            ...
            script:
                ...
                gene_trans_map=${sample_id}.gene_trans_map
                ...
                #paste a.txt a.txt >\${gene_trans_map} - does not work in container
                #touch \${gene_trans_map}
                #for x in `cat a.txt`;do echo -e \${x}"\\t"\${x} >> \${gene_trans_map} ;done
                ass_pattern="*Trinity.fa"
                if [[ \${assembly} == \${ass_pattern} ]]; then
                    get_Trinity_gene_to_trans_map.pl \${assembly} > \${gene_trans_map}
                else 
                    cat \${assembly} | awk '/>/{OFS="\t";sub(/>/, "", \$1); print \$1,\$1}' > \${gene_trans_map}
                fi
                #Get Trinotate.sqlite from folder (original)
                cp ${params.Tsql} ./\${assembly}.sqlite
                sqlname="\${assembly}.sqlite"
                #sqlname=`echo ${params.Tsql} | tr "\\/" "\\n" | grep "\\.sqlite"`

                echo -e "\\n-- Running Trinotate --\\n"

                Trinotate \$sqlname init --gene_trans_map \${gene_trans_map} --transcript_fasta \${assembly} --transdecoder_pep \${transdecoder}
		...
		# this will actually output only KO terms, not KEGG!
                ...
                echo "Perl: \$( perl -v | head -n2 | grep version | cut -f 1 -d ")" | cut -f 2 -d "(" | tr -d "v" )" >perl.version.txt
                # printf ""
                """
        }
...
#                | sed 's/\\([0-9] \\)/\\1#/g' | tr "#" "\\t" | sed -r 's/^[^0-9]*([0-9]+)/\\1,/g' > GO_cellular.csv
...
#                | sed 's/\\([0-9] \\)/\\1#/g' | tr "#" "\\t" | sed -r 's/^[^0-9]*([0-9]+)/\\1,/g' > GO_molecular.csv
                ...
                Rscript !{projectDir}/bin/GO_plots.R !{sample_id}

                rm all_GOs.txt final_GOs.txt

                # v=\$( R --version | grep "R version" | awk '{print \$3}' )
                echo "R: \$( R --version | grep "R version" | awk '{print \$3}' )" > r.version.txt
                '''
        }
        ...
        process get_transcript_dist {
            ...
            script:
                ...
                echo "Python: \$( python --version | cut -f 2 -d " " )" > python.version.txt
                """
        }
            ...
            if (params.host && params.symbiont) {
                ...
                process psytrans_run {
                    ...
                    script:
                        ...
                        # v=\$( echo 1.0.0 )
                        echo "Psytrans: 1.0.0" >psytrans.version.txt
                    	"""
                }
    ...
    if (!params.skipGetRunInfo) {
        ...
        process get_run_info {
            ...
            script:
                ...
                echo "Uniprot_DB: \$( echo ${params.uniname} )" >>.runInfo.txt

                if [ -f ${params.pipeInstall}/DBs/uniprot_db/.lastrun.txt ];then
                    echo -e "Uniprot_DB last update: \$( cat ${params.pipeInstall}/DBs/uniprot_db/.lastrun.txt ) \\n" >>.runInfo.txt
                else
                    echo -e "Uniprot_DB last update: No info available. Check Instructions on README. \\n" >>.runInfo.txt
                fi
                # echo -e "Uniprot_DB last update: \$v \\n" >>.runInfo.txt

                if [ -f ${params.pipeInstall}/DBs/hmmerdb/.lastrun.txt ];then
                    echo -e "PfamA last update: \$( cat ${params.pipeInstall}/DBs/hmmerdb/.lastrun.txt ) \\n" >>.runInfo.txt
                    # v=\$( cat ${params.pipeInstall}/DBs/hmmerdb/.lastrun.txt )
                else
                    # v="No info available. Check Instructions on README."
                    echo -e "PfamA last update: No info available. Check Instructions on README. \\n" >>.runInfo.txt
                fi
                # echo -e "PfamA last update: \$v \\n" >>.runInfo.txt

                # v=\$( echo ${params.busco4db} | tr "/" "\\n" | tail -n 1 )
                echo "BUSCO_v4_DB: \$( echo ${params.busco4db} | tr "/" "\\n" | tail -n 1 )" >>.runInfo.txt
